...
class DNAEncoder:
    ...
    def encode_patient_info(self):
        information =  str(self.files[0]['PatientName'])+'\n'+str(self.files[0]['PatientID'])+'\n'+str(self.files[0]['PatientBirthDate'])+'\n'+str(self.files[0]['PatientAge'])+'\n'+str(self.files[0]['PatientSex'])
        ascii_information=[]
        int_list = []
    ...
    def encode_char(self, x, target_len):
        num = ord(x) 
        res = ""
        nuc = ['A', 'T', 'C', 'G']
        for i in range(target_len):
            res = nuc[num & 0b11] + res
            num >>= 2
        return res

    def encode_extra_info(self, info_seq):
        self.info_seq = info_seq
        self.DNA_info_seq = ''
        for c in info_seq:
            self.DNA_info_seq = self.DNA_info_seq + self.encode_char(c, 4)
        return self.DNA_info_seq


    def encode(self,out_file,user_name,password, extra_info):
        self.user_name = user_name
        self.password = password
        self.DNA = ""
        self.DNA += primerEncoder.primerencode(user_name, password)
        self.DNA += self.encode_patient_info()
        self.DNA += self.encode_image()
        if extra_info != '':
            self.DNA += self.encode_extra_info(extra_info)
...
class DNADecoder():
    ...
    def decode_img(self, DNA_sequence, out_dir, prefix):
        ...
        self.img_array_3D = np.zeros((self.x, self.y, self.z), dtype='uint16')
        for i in range(self.z):
            ...
            self.img_array_3D[:,:,i] = img_array
            plt.imshow(img_array, cmap=plt.cm.gray)
            plt.savefig(file_name)
            print(i) 
    
    def decode_extra_info(self, information_DNA, out_dir,prefix):
        string_information = ''
        for i in range(int(len(information_DNA)/4)):
            DNA_4_i = information_DNA[4*i:4*i+4]
            bin_i = ''
        ...
        file_name = os.path.join(out_dir, prefix + '.extra_info.txt')
        f = open(file_name,'w')
        # print(string_information)
        f.write(string_information)
        f.close()
    ...
    def decode(self, dna_file,out_dir,prefix):
        ...
        f = open(dna_file, 'r')
        self.DNA_total = f.read()
        self.DNA_length = len(self.DNA_total)
        ...
        self.image_length = 24 + 8* self.x * self.y * self.z
        if 108 + self.patient_info_length*4 + self.image_length + 100 == self.DNA_length:
            print(" ")
        else:
            self.decode_extra_info(self.DNA_total[108 + self.patient_info_length*4 + self.image_length:-100],out_dir,prefix)
...
# primer_file = 'hospital_primer.txt'
...
# encoder.encode("dna.x.txt",username, password, extra_info)
...
# decoder.decode('dna.x.txt','./decode_output','result')
